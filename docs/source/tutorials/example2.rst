...
for example for the decision tree on the first view the ``*-hps_report.txt`` file generated by the randomized search of :base_doc:`example 2.2 <tutorial/example2.html#example-2-2-usage-of-randomized-hyper-parameter-optimization>` looks like :
...
So to run a decision tree with these exact parameters, one just has to follow the method of :base_doc:`example 2.1 <tutorial/example2.html#example-2-1-no-hyper-parameter-optimization-impact-of-split-size>` and run |platf| with the following hyper-parameter configuration :
