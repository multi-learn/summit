...
class AdaboostPregen(AdaBoostClassifier, BaseMonoviewClassifier, PregenClassifier):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        self.base_predictions = np.array([change_label_to_zero(estim.predict(pregen_X)) for estim in self.estimators_])
        self.metrics = np.array([self.plotted_metric.score(change_label_to_zero(pred), y) for pred in self.staged_predict(pregen_X)])
    ...
    def getInterpret(self, directory, y_test):
        ...
        get_accuracy_graph(step_test_metrics, "AdaboostPregen", directory + "test_metrics.png",
                           self.plotted_metric_name, set="test")
        get_accuracy_graph(self.metrics, "AdaboostPregen", directory+"metrics.png", self.plotted_metric_name, bounds=list(self.bounds), bound_name="boosting bound")
